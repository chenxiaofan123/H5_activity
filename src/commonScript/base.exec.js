...
GB.utils = {
    ...
    isSpeScreen:function(){
      var screen_proportion = window.screen.width / window.screen.height;
      if(screen_proportion<=0.486){
         return true;
      }else{
         return false;
      }
    ...
    //调起交易记录
    showFinanceWater: function () {
        var params_phone = {
        };
        GB.utils.callCustomMethod("showFinanceWater", params_phone);
    },
    toast: function (msg) {
        var data = { 'msg': msg };
        this.send({
    ...
    isDev: function () {
        var isDev = /test-app-activity\.g-banker/ig.test(location.href);
        return isDev;
    },
